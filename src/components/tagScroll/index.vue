  ...
  <div class="overflow-hidden flex items-center">
    ...
    <div class="flex-1 overflow-hidden px-5 flex-nowrap" ref="scrollbarDom">
      <div class="flex flex-nowrap gap-8 overflow-visible" ref="tabDom" :style="getTabStyle">
        <el-check-tag v-for="(item, index) in options" :checked="item.checked"
          @change="(val: any) => changeItem(val, item)" :ref="'dynamic' + index" :key="index" size="small">
          {{ item.name }}
        </el-check-tag>
      </div>
    </div>
...
const getTabStyle = computed(() => { return { transform: `translateX(${translateX.value}px)` } })
