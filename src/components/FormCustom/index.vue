<template>
  <div>
    <el-form v-bind="$attrs" ref="ruleFormRef" v-model="props.formDate" :rules="props.rules">
      <el-row>
        <el-col v-for="(item, index) in formColumn" :key="index + 'sd'" v-bind="item.col">
          <template v-if="item.slotName">
            <slot :name="item.slotName" :item="item"></slot>
          </template>
          <el-form-item v-else :label="item.label" :prop="item.prop" v-bind="item.formItemProps">
            <component :is="item.type" v-bind="item.componentsProps" v-model="props.formDate[item.prop]">
              <template v-for="(index, name) in slots" :key="index + 'gg'" #[name]>
                <slot :name="name" :item="item"></slot>
              </template>
            </component>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
  </div>
</template>

<script setup lang="ts">
import { useSlots, reactive, ref } from 'vue'
import { defaultProps } from './props'
import type { FormInstance } from 'element-plus'
const slots = useSlots()
const props = defineProps(defaultProps)

// 将表单绑定的ref暴露给父组件
const ruleFormRef = ref<FormInstance>();
defineExpose({ ruleFormRef });

</script>