    ...
    <template #footerLeft>
      <div class="w-full flex justify-start gap-x-8">
        <span v-for="(index, item) in OBJICON" :key="index">
          <span class=" inline-flex items-center justify-center w-28 h-28 bg-white border rounded shadow-2xl"
            :class="{ 'rotate-180': item === '向上' }">
...
const routerPush = () => {
  const routeAcitve = menus.value[isActive.value]
  if (routeAcitve && routeAcitve.path) {
    router.push(routeAcitve.path)
    store.commit(RootMutations.SET_TAGS, {
      ...routeAcitve,
      name: routeAcitve.meta.title,
      path: routeAcitve.path,
      type: '',
      color: '#fff'
    })
    handlerClose()
