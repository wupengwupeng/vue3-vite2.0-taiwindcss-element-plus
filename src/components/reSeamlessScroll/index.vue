...
if ((classOption as any)['key'] === undefined) {
  ;(classOption as any)['key'] = 0
}

const wrap = templateRef<HTMLElement | null>(`wrap${(classOption as any)['key']}`, null)
const slotList = templateRef<HTMLElement | null>(`slotList${(classOption as any)['key']}`, null)
const realBox = templateRef<HTMLElement | null>(`realBox${(classOption as any)['key']}`, null)
...
let scrollSwitch = computed(() => {
  ...
  return (props as any).data.length >= unref(options).limitMoveNum
})
...
function scrollInitMove() {
  nextTick(() => {
    ...
    if (unref(scrollSwitch)) {
      ...
      setTimeout(() => {
        realBoxHeight.value = (unref(realBox) as any).offsetHeight
        scrollMove()
      }, 0)
    } else {
      scrollCancle()
...
<template>
  <div :ref="'wrap' + (classOption as any)['key']">
    ...
    <div
      :ref="'realBox' + (classOption as any)['key']"
      :style="pos"
      @mouseenter="enter"
      @mouseleave="leave"
      ...
      @mousewheel="wheel"
    >
      <div :ref="'slotList' + (classOption as any)['key']" :style="float">
        <slot></slot>
      </div>
